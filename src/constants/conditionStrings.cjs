 ...
 * startsWith, includes, array membership). CommonJS so main, preload, and Vite
 * renderer can share the same values.
 *
...
const HISTORY_TRANSITION = freeze({
  TYPED: 'TYPED',
  RELOAD: 'RELOAD',
  BOOKMARK: 'BOOKMARK',
  LINK: 'LINK',
...
const OMNIBOX_SUGGESTION = freeze({
  HISTORY: 'history',
  SEARCH: 'search',
  BOOKMARK: 'bookmark',
  KEYWORD: 'keyword',
...
const HISTORY_VIEW = freeze({
  CHROME_HISTORY: 'chrome-history',
});

/** New tab tile kinds */
const NTP_TILE = freeze({
  SITE: 'site',
});

/** WebRequest resource types */
const RESOURCE_TYPE = freeze({
